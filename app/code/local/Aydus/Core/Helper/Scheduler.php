...
class Aydus_Core_Helper_Scheduler extends Mage_Core_Helper_Abstract 
{
	...
	public function generateSchedules($value, $jobCode, $frequency, $monthDay, $weekDay, $startTime) 
	{
		$this->_value = $value;
		...
		if ($schedulesCollection->getSize()>0){
			...
			Mage::log($jobCode.' scheduled at '.$message, null, 'aydus_core.log');
		}	
	...
	public function schedule($H, $i, $s, $m, $d, $Y)
	{
		...
		try {
			...
			$schedule = Mage::getModel('aydus/schedule');
			$schedule->setValue($this->_value)
			...
			Mage::log($e->getMessage(), null, 'aydus_core.log');
			return $e->getMessage();
		}		
	}
